...
import { CartItemDto, CartItemSchemaDto, CartItemUpdateDto } from '../dtos/cart.item.dto';
 ...
 */
const createCartItem = (dto: CartItemDto, user: Types.ObjectId): Promise<CartItemSchemaDto> => {
 ...
 */
const deleteCartItem = async (slug: string): Promise<CartItemSchemaDto | null> => {
 ...
 */
const fetchAllCartItem = async (user?: Types.ObjectId): Promise<CartItemSchemaDto[]> => {
 ...
 */
const fetchCartItem = async (slug: string): Promise<CartItemSchemaDto | null> => {
 ...
 */
const updateCartItem = async (slug: string, dto: CartItemUpdateDto): Promise<CartItemSchemaDto | null> => {
  // Find the cartItem by its slug
  const cartItem = await cartItemRepository.findOneBySlug(slug);
