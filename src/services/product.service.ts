...
import { ProductDto, ProductSchemaDto, ProductUpdateDto } from '../dtos/product.dto';
 ...
 */
const createProduct = (dto: ProductDto, user: Types.ObjectId): Promise<ProductSchemaDto> => {
 ...
 */
const deleteProduct = async (slug: string): Promise<ProductSchemaDto | null> => {
 ...
 */
const fetchAllProduct = async (user?: Types.ObjectId): Promise<ProductSchemaDto[]> => {
 ...
 */
const updateProduct = async (slug: string, dto: ProductUpdateDto): Promise<ProductSchemaDto | null> => {
  // Find the product by its slug
  const product = await productRepository.findOneBySlug(slug);
