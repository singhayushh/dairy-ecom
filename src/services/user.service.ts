...
import { UserCacheDto, UserDto, UserSchemaDto, UserUpdateDto } from '../dtos/user.dto';
 ...
 */
const signIn = async (email: string, password: string): Promise<{ token: string; dto: UserCacheDto } | null> => {
  ...
  const dto: UserCacheDto = {
    ...
    role: user.role,
    isAdmin: (role && role._id.toString() === user.role?.toString()) ?? false,
  };
 ...
 */
const fetchAllUser = async (customerOnly: boolean = true): Promise<UserSchemaDto[]> => {
  if (customerOnly) {
 ...
 */
const updateUser = async (id: Types.ObjectId, dto: UserUpdateDto): Promise<UserSchemaDto | null> => {
